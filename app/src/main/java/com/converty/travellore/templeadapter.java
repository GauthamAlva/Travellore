...
public class templeadapter extends ArrayAdapter<temple> {
    public templeadapter(Activity context, ArrayList<temple> templeArrayList){
        // Here, we initialize the ArrayAdapter's internal storage for the context and the list.
        // the second argument is used when the ArrayAdapter is populating a single TextView.
        // Because this is a custom adapter for two TextViews and an ImageView, the adapter is not
        // going to use this second argument, so it can be any value. Here, we used 0.
        super(context,0,templeArrayList);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(listItemView == null) {
            listItemView = LayoutInflater.from(getContext()).inflate(
                    R.layout.templedesign, parent, false);
        }

        // Get the {@link AndroidFlavor} object located at this position in the list
        temple current = getItem(position);
       ...
       info.setText(current.getinfo());
        ...
        ImageView img=(ImageView) listItemView.findViewById(R.id.imagetemp);
